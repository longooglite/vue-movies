...
const populateOptionalFields = (searchMovieResult: SearchMovieResult): Movie => {
    return Object.entries({
        title: searchMovieResult?.title,
        year: Number(searchMovieResult.release_date.split('-')[0]),
        director: searchMovieResult?.director ? searchMovieResult.director.name : '',
        ...
        acc[key] = value
        return acc
    }, {} as Record<string, string | number | boolean | undefined>) as Movie
}

export const searchResultToAWSMovie = (searchMovieResult: SearchMovieResult): Movie => {
