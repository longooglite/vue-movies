...
import { movieFilterProviderKey, MovieFilterProviderReturn } from '../../../types/providers/MovieFilterProvider'
...
const currentFilterValue = computed(() => {
    const rawValue = filterProvider.filters.value.find((filter) => filter.key === props.filterKey)?.value
...
<template>
    <div class="dropdown-filter" @click="toggleOpen">
        ...
        <span v-show="!open" class="dropdown-filter-value">{{ currentFilterValue.toString() }}</span>
        <button>
